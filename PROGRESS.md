...
- [x] Phase 2.1 — Subscription auth alongside BYO API key (flag-gated)  ← live subscription run still to verify
...
- **⏸️ Subscription auth is operator-only until Anthropic approves (Phase 2.1).** Code-complete and CI-green behind `ALLOW_SUBSCRIPTION_AUTH`, but the Agent SDK docs state that third-party developers may not offer claude.ai login or rate limits for their products unless previously approved (checked 2026-09-02). Using our own subscription for our own installations is fine; offering it on the setup page is not, so 12b still writes API keys only. Also unverified: the memory envelope on Render, since the SDK spawns its bundled Claude Code binary per model call and the CocoIndex sidecar already spikes ~300MB on a 512MB Starter.
